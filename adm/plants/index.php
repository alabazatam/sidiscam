		...
		if(count($plants_list_json)>0)
		{
			foreach ($plants_list_json as $plant) 
			{
				...
				if($status == 0)
				{
                                        ...
                                        if($_SESSION['rol'] == "ADM")
                                        {
                                            $onclick = "onclick = ".'"'."status_changer('plants','id_plant', '$id_plant','1')".'"'."";
				...
				if($status == 1)
				{
                                        ...
                                        if($_SESSION['rol'] == "ADM")
                                        {
                                            $onclick = "onclick = ".'"'."status_changer('plants','id_plant', '$id_plant','0')".'"'."";
				...
				$array_json['data'][] = array(
					...
					"name" => $plant['name'],
					"rif" => $plant['rif'],
					"contact1" => $plant['contact1'],
        				"phone_contact1" => $plant['phone_contact1'],
    					"email_contact1" => $plant['email_contact1'],
			...
			$array_json['data'][0] = array("id_plant"=>null,"name"=>"","rif"=>"","contact1"=>"","status"=>"","date_created"=>"","date_updated"=>"","actions"=>"");
		}
